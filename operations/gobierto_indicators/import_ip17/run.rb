...
# This script imports Sant Feliu Infoparticipa 2017 indicators
#
# Usage:
#
...
# Samples:
#
#   /path/to/gobierto/bin/rails runner /path/to/project/operations/gobierto_indicators/import_ip17/run.rb domain.gobierto.test
...
puts "[START] import_ip17/run.rb with domain: #{domain}"
...
response = client.call(:consulta_indicadors, message: { peticio: {indicadornom: 'IP17'}})
...
indicators.each do |indicator|
  ...
  @ip17[0][:children][section][:children].push({ id: id, level: 2, attributes: { title: title, checked: value, links: links }})
end
...
puts "[DEBUG] Generating Indicators Infoparticipa 17..."

GobiertoIndicators::IndicatorsImporter.new("ip", @site, @ip17.to_json, @year).import!

puts "[END] import_ip17/run.rb"

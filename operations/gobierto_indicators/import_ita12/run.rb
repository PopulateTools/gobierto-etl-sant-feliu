...
# This script imports Sant Feliu ITA 2012 indicators
#
# Usage:
#
...
# Samples:
#
#   /path/to/gobierto/bin/rails runner /path/to/project/operations/gobierto_indicators/import_ita12/run.rb domain.gobierto.test
...
puts "[START] import_ita12/run.rb with domain: #{domain}"
  ...
  },
  {
    ...
    "children": [
      ...
      },
      {
        ...
        "level": 1,
        "attributes": {
          "title": "C.3.- Transparència en els deutes municipals"
        },
        "children": []
      }
    ]
  ...
  },
  {
    ...
    "children": [
      ...
      },
      {
        ...
        "level": 1,
        "attributes": {
          "title": "E.5.- Seguiment i control de l´execució d´obres"
        },
        "children": []
      },
      {
        ...
        "level": 1,
        "attributes": {
          "title": "E.6.- Indicadors sobre urbanisme i obres públiques"
        },
        "children": []
      }
    ]
...
response = client.call(:consulta_indicadors, message: { peticio: {indicadornom: 'ITA12'}})
indicators = response.body.first.last.first.last.first.last

indicators.each do |indicator|
  indicador_nom = indicator[:indicadornom]
  indicador_nom_splitted = indicador_nom.split("ITA12.").last.split(".")
  ...
  value = response.body.first.last.first.last.first.last.key?(:valor) && !["243", "248", "373", "380"].include?(indicator[:indicadordef])
  ...
  if indicator[:indicadordesc] != "Indemnitzacions percebudes amb ocasió de l'abandonament dels càrrecs"
    @ita12[ita_letter][:children][ita_number][:children].push({ id: id, level: 2, attributes: { title: title, checked: value, links: links }})
  end
end

# Order arrays

@ita12.each do |letter|
  letter[:children].each do |number|
    number[:children] = number[:children].sort_by { |hsh| hsh[:id] }
  end
end

puts "[DEBUG] Generating indicators ITA 12..."

GobiertoIndicators::IndicatorsImporter.new("ita", @site, @ita12.to_json, @year).import!

puts "[END] import_ita12/run.rb"

...
# This script imports Sant Feliu Infoparticipa 2016 indicators
#
# Usage:
#
...
# Samples:
#
#   /path/to/gobierto/bin/rails runner /path/to/project/operations/gobierto_indicators/import_ip16/run.rb domain.gobierto.test
...
puts "[START] import_ip16/run.rb with domain: #{domain}"
...
response = client.call(:consulta_indicadors, message: { peticio: {indicadornom: 'IP16'}})
...
indicators.each do |indicator|
  ...
  @ip16[0][:children][section][:children].push({ id: id, level: 2, attributes: { title: title, checked: value, links: links }})
end
...
puts "[DEBUG] Generating Indicators Infoparticipa 16..."

GobiertoIndicators::IndicatorsImporter.new("ip", @site, @ip16.to_json, @year).import!

puts "[END] import_ip16/run.rb"

...
# This script imports Sant Feliu ITA 2017 indicators
#
# Usage:
#
...
# Samples:
#
#   /path/to/gobierto/bin/rails runner /path/to/project/operations/gobierto_indicators/import_ita17/run.rb domain.gobierto.test
...
puts "[START] import_ita17/run.rb with domain: #{domain}"
  ...
  },
  {
    ...
    "children": [
      ...
      },
      {
        ...
        "level": 1,
        "attributes": {
          "title": "D.2.- Convenis, encàrrecs de gestió i subvencions"
        },
        "children": []
      },
      {
        ...
        "level": 1,
        "attributes": {
          "title": "D.3.- Subministradors i costos dels serveis"
        },
        "children": []
      }
    ]
...
response = client.call(:consulta_indicadors, message: { peticio: {indicadornom: 'ITA17'}})
indicators = response.body.first.last.first.last.first.last

indicators.each do |indicator|
  # Hack to load indicators well
  indicador_nom = indicator[:indicadornom].gsub("ITA17.I.", "ITA17.E.")
  indicador_nom_splitted = indicador_nom.split("ITA17.").last.split(".")
  ...
  if !response.body[:consulta_valors_indicador_response][:return].nil?
    value = !((response.body.first.last.first.last.first.last[:valor] == "En desenvolupament") ||
              (response.body.first.last.first.last.first.last[:valor].nil?))
    indicator_splitted = indicator[:indicadornom].split(".")
    id = indicator_splitted[3].to_i
    ...
    @ita17[ita_letter][:children][ita_number][:children].push({ id: id, level: 2, attributes: { title: title, checked: value, links: links }})
  end
end

# Clean indicators

@ita17.each do |letter|
  letter[:children].delete_if { |k, v| k[:children].empty? }
end
@ita17.delete_if { |k, v| k[:children].empty? }
...
puts "[DEBUG] Generating indicators ITA 17..."

GobiertoIndicators::IndicatorsImporter.new("ita", @site, @ita17.to_json, @year).import!

puts "[END] import_ita17/run.rb"
